...
\item On the support-visible compact-gauge branch, OPH first derives the four-dimensional
Euclidean Yang--Mills form from compact-gauge holonomy data and the local MaxEnt/Gibbs
continuum limit. Local repair collars then generate Euclidean time through conditional
...
\begin{proof}
Use the finite gauge-register / quantum-link reference architecture on the zero-obstruction branch. At a sufficiently fine compact cutoff, retain the Peter--Weyl labels listed in \(\mathcal W_{\mathrm{SM}}\). They are finite-dimensional unitary representations of \(\SU(3)\times\SU(2)\times\U(1)\), and with integer normalization \(q=6Y\) Proposition~\ref{prop:z6} shows that the generator \(g_0=(\omega_3,-1,e^{i\pi/3})\) acts trivially on exactly the displayed matter and Higgs multiplets. The witness therefore descends to the quotient \((\SU(3)\times\SU(2)\times\U(1))/\mathbb Z_6\).
...
finite-cutoff Casimir branch itself is not a missing input.
...
does not by itself produce a worldsheet genus expansion. It is not the four-dimensional
compact-gauge repair-gap theorem. The separate support-visible repair-dynamics result is
