...
\]
The physical OPH theorem assumes faithful trace-preserving u.c.p. maps \(\Phi\) and \(\Psi\) from
the source process to a physical response process and back, with \(\Psi\Phi=\mathrm{id}\).
Kadison--Schwarz applied to both maps puts every source element in the
...
Koide. Construction of these maps, the chiral regular-\(C_3\) carrier, refinement naturality,
positivity, kinetic descent, mass-scheme descent, and no-target-leak ancestry is work in progress.
