...
surface and the \(\mathcal R_U\) precision policy. Promotion of the naturality reading also
requires the RG/coarse-graining Higgs readout defect \(\epsilon_H\) to vanish, or to be bounded
inside the declared Higgs output interval.
...
exponent without correction.
\end{lemma}

\begin{proof}
The D6 normalization gives
\(\rho_\star=\ell_\star/r_{\mathrm{CRC}}=(N_{\mathrm{CRC}}/\pi)^{-1/2}\). The closure
condition is not an independent hypothesis: the Observers synthesis defines the readback map
as
...
scale-covariant extension of \(N=\pi(r/\ell)^2\), a declared identification rather than a
corpus-stated property — the readback reconstructs
...
branch; the closure transport is derived at the interface level of \(F\) under the declared
area-law counting model of \(\operatorname{Cap}_{\mathrm{read}}\), while the round
count itself is the declared branch architecture rather than a derived integer. A
theorem-grade promotion of the resonance relation still requires the finite-machinery
verification that \(\operatorname{nf}_{r,N}\) delivers a single well-defined effective
resolution counted by that model (the corpus marks the finite readback map as schematic), a
representation-to-spectrum theorem deriving that round count from the OPH repair grammar, an
...
coordinates rather than the selected normal-form readout. The relation is therefore still a
continuation target connecting \(\mathcal R_U\) and \(\mathcal R_N\), not an input to the
promoted hierarchy row.
