...
fixed point of the declared numerical map; CL-6, closed 2026-07-14).
...
with the electroweak bridge only conditionally on CP-1 to CP-3. A separate count-density candidate is
\[
N_\star=\operatorname{MAR}\arg\max_N\left[\log|\Omega^{\mathrm{sc}}_N|-N\right],
\]
with a differentiable mathematical map given by
...
\(N_{\mathrm{CRC}}\) alone.
The separate count-density candidate is
\[
N_\star=\operatorname{MAR}\arg\max_N\left[\log|\Omega^{\mathrm{sc}}_N|-N\right].
\]
...
convention. No pull or physical near-hit is assigned to these noncommensurate coordinates.
...
complex-pole masses use another convention. The coordinates therefore have no valid pull or
physical near-hit interpretation. Numerical \(W/Z\) proximity cannot decide between the laws;
