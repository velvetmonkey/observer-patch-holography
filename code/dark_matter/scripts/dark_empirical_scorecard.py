...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "The SPARC calculations are empirical parameterization checks. "
            "The CMB, BAO, growth, and S8 calculations use imported background "
            "inputs and a diagnostic scalar-load component. The cluster calculation "
            "is an offset-timing scaffold. None is a prediction or likelihood score "
            "of the repair-charge condensate proposal.",
            "",
            "## Declared external inputs",
            "",
            f"- Neutrino mass sum: {inputs['sum_mnu_eV']:.9f} eV",
            f"- Neutrino hierarchy: {inputs['neutrino_hierarchy']}",
            f"- Neutrino input status: "
            f"{inputs['neutrino_mass_input_provenance']['status']}",
            "",
            "## Physical comparison boundary",
            "",
            "Physical comparison requires a source-derived repair-charge action, "
            "dimensional source normalization, relativistic stress and exchange "
            "closure, cosmological abundance and initial perturbations, lensing and "
            "disk dynamics, a Boltzmann implementation, cluster maps, and complete "
            "likelihoods. These derivations are work in progress.",
            "",
        ]
    )
