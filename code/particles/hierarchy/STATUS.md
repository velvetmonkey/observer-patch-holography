...
| `R_readback_resolution_certificate.json` | closed finite readback resolution | Defines `F_r(N)=Cap_read(Obs(nf_{r,N}(U_{r,N})))` and `rho_read(r,N)=sqrt(pi/F_r(N))`; the selected finite branch has one positive central capacity atom, zero variance, and one delivery resolution, with positive-root refinement limit `(N_CRC/pi)^(-1/2)`. |
